...
"""Asyncio requires the application code to handle context changes based on the idea of an event loop.
    ...
    Applications interact with the event loop to register code to run and let the loop call the code when
    resources become available. e.g. registration of a network socket then awaits inbound connections and
    calls back to application code when the event is fired.

    Application code then yields control when no more work is to be done, in the example it could be after
    an ack is sent.
...
# here we are chaining coroutines together to make a task easier to decompose into
# reusable parts.
async def outer():
    print('in outer')
